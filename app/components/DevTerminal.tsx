          ...
          <div className="flex items-center gap-2 text-[13px] text-green-400">
            ...
            <div className="flex-1 relative">
              <input
                ...
                onKeyDown={onKeyDown}
                placeholder={
                  lines.length === 1 ? "Type a command... (/help)" : ""
                }
                className="bg-transparent w-full outline-none text-gray-100 placeholder-gray-500 caret-transparent"
                aria-label="Terminal input"
                autoComplete="off"
                spellCheck={false}
              />
              {/* Green flickering cursor */}
              <span
                className="absolute top-0 bg-green-400 w-2 h-4 opacity-80"
                style={{
                  left: `${Math.max(0, current.length - 1) * 0.6}em`,
